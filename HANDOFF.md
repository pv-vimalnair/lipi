...
*End of handoff. Lipi is at **Phase 9.30a complete** (`lsp_check_available` per-kind dispatch + `kindToSpawnSpec` parity — Rust `LspServerKind` enum + `CheckAvailableArgs` struct + `server_kind_spec()` / `check_available_for()` helpers added in `src-tauri/src/stdio.rs`, `lsp_check_available` Tauri command now takes an optional `args: Option<CheckAvailableArgs>`, defaults to `Typescript` when missing for backward compat, per-kind install hints (`npm install -g typescript-language-server` / `rustup component add rust-analyzer` / `npm install -g pyright`); TS side: `LspServerKind` union + `CheckAvailableArgs` interface + `LspSpawnSpec` interface + `kindToSpawnSpec()` pure function + `lspCheckAvailable(args?)` wrapper that forwards the kind to the Tauri command; new `src/ipc/lsp.test.ts` (10 tests) pins the JS spawn spec ↔ Rust `server_kind_spec` table parity contract; D-147 "two tables that must agree on binary name + install hint" + D-148 "`lspCheckAvailable` is total over the full union including `'unknown'`" decisions documented; total 1162 TS + 387 Rust tests, vitest + tsc + cargo check + cargo test --lib stdio (29) all clean). The next session should resume from the **Phase 9.2f follow-ups** (D-145 `EditorPane` single-Monaco refactor + D-146 provider respawn re-registration), then the parked M6c / M3 follow-up / mobile-build roadmap, plus the two items from §9.30 (MSI bundling regression, `whisper-rs` bump) and the JSON/CSS/HTML workers follow-up in §9.31. The next handoff entry will live at §9.46.*
