...
**Current state (as of 2026-06-19).** Production-ready foundation
...
**Most recent shipped phase.** **Phase 10** — Editor tab theme: 5
vintage scenes. See §9.52 below. Total decisions numbered
#46–#189 (with #187–#189 added in Phase 10). Total Rust tests 413 + 24
integration = 437. Total JS tests 1293 across 99 files.
Post-Phase-10 IDE-readiness hardening has also landed for native STT
build health, AI file-read scoping, tool confirmation defaults, raw-key
renderer access, Android Stronghold fail-closed behavior, production
private key removal from the repo tree, custom-tool registry cleanup,
Rust-side IPC policy gates + audit logging for shell/LSP/HTTP/terminal,
HTTP body caps and host/private-network policy,
deep-link validation, workspace-search ignores and cancellation,
shell-timeout process cleanup, lint wiring, and generated TypeScript
build artifacts. Dev-tool audit findings are also cleared on the Vite 8
toolchain; Node `>=20.19.0` is now the supported JS runtime floor.
...
4. **§9 below** for the phase-by-phase history with file lists and
   decisions. The latest phase is §9.52.
...
```
Expected: `tsc` 0 errors, `vitest` 1293/1293, `vite build` clean,
`cargo check` 0 errors 0 warnings, `cargo test --lib` 413/413,
...
**The m2c-native on-device STT path — buildable (real cpal + whisper inference wiring is a separate follow-up, see §9.30b Phase 6.2)**

The on-device STT path (M2c) requires `cargo build --features m2c-native`, which in turn requires `libclang.dll` (LLVM) + `cmake` on the build machine. Both are installed: LLVM 22.1.7 (434 MB, full Windows installer) and CMake 4.3.3 (51 MB portable zip, extracted to `C:\Users\Pv Vimal Nair\Tools\cmake-4.3.3-windows-x86_64\`). Phase 6.2 bumped `whisper-rs` from `0.14` to `0.16` (Decision #164), which pulls in `whisper-rs-sys 0.15.0` (was 0.13.1) and a matching size assertion. The build now succeeds end-to-end (`cargo check --features m2c-native --lib` is clean, `cargo test --lib --features m2c-native` is 358/358 pass). What is NOT yet done: wiring the real cpal mic capture and the real `WhisperContext::new_with_params` + `WhisperState::full` inference call. The `m2c-native` `start_listening` / `stop_listening` / `install_model` bodies are still stubs that just `Ok(())` — Decision #166. The daily-driver STT today is Web Speech (which is fully functional); the desktop cpal + whisper path is a one-day follow-up on a real machine. The full Phase 6.2 investigation and verification table are in §9.30b.
...
1. **m2c-native on-device STT** — RESOLVED in §9.30b / Phase 6.2 (Decision #164). The build is now compile-clean end-to-end. The remaining piece is wiring the real cpal + whisper inference (Decision #166), which is a one-day follow-up on a real machine.
...
The M6c / M3 follow-up / mobile-build roadmap parked at the end of the production-readiness pass is now unblocked. The next session picks up there.
...
### 9.30b Phase 6.2 — SHIPPED (`whisper-rs` 0.14 → 0.16, the m2c-native build now compiles, see CHANGELOG "Changed (Phase 6.2 — `whisper-rs` 0.14 → 0.16)")

The §9.30 hand-off noted that `cargo build --features m2c-native` was broken because `whisper-rs-sys 0.13.1` (transitively pulled in by `whisper-rs = "0.14"`) failed with `error[E0080]: attempt to compute 1_usize - 264_usize, which would overflow` at the bindgen static assertion. The §9.30 root-cause hypothesis was "whisper.cpp restructured `whisper_full_params`; bindgen produced an empty struct; the 264-byte assertion underflowed." That diagnosis was directionally correct, and Phase 6.2 is the upstream-bump fix.

**What was actually wrong.** `whisper-rs-sys 0.13.1` vendors a `bindings.rs` (the *static* one) whose size-of-`whisper_full_params` assertion (`["Size of whisper_full_params"][::std::mem::size_of::<whisper_full_params>() - 264usize]`) was written against the pre-v1.8 whisper.cpp struct layout (264 bytes). The build script's `bindgen` invocation against the current `whisper.h` header produces a `whisper_full_params` struct with **`_address: u8` only** (size 1) because the v1.8+ header now has a forward declaration `struct whisper_full_params;` (line 82 of `whisper.h`, inside the `extern "C"` block) that bindgen picks up before reaching the full struct body (line 476). The forward declaration is the C-only-build guard that lets `whisper.h` be `extern "C"`-includeable without dragging in the full struct body for callers that only need the API surface. The 264-byte assertion then underflows (1 − 264) and the build dies with `E0080`.

**Why the vendored `bindings.rs` had the full struct.** whisper-rs-sys 0.13.1's `src/bindings.rs` was generated against a whisper.cpp commit *before* the forward-declaration was added. The build script always re-runs `bindgen` (and falls back to the vendored one only on bindgen failure), so the newer header trips the stale vendored assertion. The fix is to bump the whisper-rs version so that whisper-rs-sys vendors a newer whisper.cpp + a matching size assertion, OR to add a `WHISPER_DONT_GENERATE_BINDINGS=1` escape hatch that uses the vendored file (which the build.rs already supports, but the vendored one has a different field set than what whisper.cpp v1.7+ ships — so this would be a future, much-larger API surface change, not a one-line workaround). The bump is the right move.

**Bump outcome.** `whisper-rs = "0.14"` → `whisper-rs = "0.16"` (latest stable, 2026-03-12). This pulls in `whisper-rs-sys 0.15.0` (was 0.13.1), which vendors a newer whisper.cpp + a regenerated static `bindings.rs` whose size assertion matches the actual struct layout. **No source code in `src-tauri/` needed to change** — our `m2c-native` paths are still stubs that just `Ok(())` / `Ok(session_id)`, so the API rename (`WhisperContext::new` → `WhisperContext::new_with_params` + `WhisperContext::create_state` + `WhisperState::full`) is forward-compatible: when we wire the real cpal + whisper inference later, we'll use the new API. The Cargo lockfile now resolves:

```
Updating whisper-rs v0.14.4 -> v0.16.0
Updating whisper-rs-sys v0.13.1 -> v0.15.0
```

**Verification (Windows, dev build)**

| Command | Result |
|---------|--------|
| `cargo check --features m2c-native --lib` | clean (0 errors, 7 pre-existing stub warnings in `stt_capture.rs`) |
| `cargo test --lib --features m2c-native` | 358/358 pass |
| `cargo test --lib` (default, no features) | 358/358 pass |
| `npx tsc --noEmit` | 0 errors |
| `npx vitest run` | 1185/1185 pass (4 pre-existing unhandled-rejection log lines from `LanguageServerCard.test.tsx` + `useInlineEditOverlay.test.tsx` — pre-Phase-6.2 noise from Vitest 4.x's unhandled-error detection on background listeners; not introduced by this change) |

The 7 stub warnings in `stt_capture.rs` are all in code that is `#[cfg(feature = "m2c-native")]` and is intentionally unused today (the real cpal + whisper path is a follow-up, not a blocker — Web Speech is the daily-driver STT). They are: `unused import: Emitter`, `unused import: STT_EVENT_TRANSCRIPT`, `type LiveSession is more private than the item SessionRegistry`, `struct LiveSession is never constructed`, `associated function new is never used`, `type alias SessionRegistry is never used`, `function now_ms is never used`. All predate Phase 6.2 and are noise from `m2c-native` being buildable for the first time.

**One real test fix.** With `m2c-native` now buildable, the `stt::tests::list_installed_models_includes_every_curated_id_in_stub_mode` test in `src-tauri/src/stt.rs` flipped from "pass" to "fail" because the `m2c-native` branch of `list_installed_models` filters by `is_model_installed` (no files on disk → empty list) while the stub branch returns every curated id. The test was deliberately named `_in_stub_mode` with a docstring that said "this is the only thing that doesn't depend on `m2c-native` being built." Phase 6.2 gates the test with `#[cfg(not(feature = "m2c-native"))]` (it asserts the stub-shape contract) and adds a sibling test `list_installed_models_is_empty_when_no_models_on_disk_in_real_path` gated with `#[cfg(feature = "m2c-native")]` (it pins the real-path contract: an empty app data dir → empty installed list, not "every curated id"). Both pass; the function behaviour is unchanged.

**What did NOT change.** The stub mode (no `m2c-native`) still ships the same `Ok(())` / placeholder text / no-network behaviour it did before — the bump is purely additive for the real-path build. The curated model list, the `active_model.json` file, the install/remove/set-active lifecycle, and the JS contract (`stt://download-progress`, `stt://transcript`, `stt://error` events) are all unchanged. The M3 Swift `SFSpeechRecognizer` plugin (mobile) is the next voice follow-up, not the desktop cpal + whisper integration — that one needs a real microphone and is parked.

**Decisions added by this phase**

| # | Decision | Rationale | Date |
|---|----------|-----------|------|
| 164 | Bump `whisper-rs` from `0.14` to `0.16` (pulls in `whisper-rs-sys 0.13.1` → `0.15.0`). | The §9.30 m2c-native build was broken because whisper.cpp v1.8+ added a forward-declaration of `whisper_full_params` in the header that bindgen now picks up first, producing an opaque struct (size 1) and tripping the static 264-byte size assertion. whisper-rs 0.16 ships a matching size assertion for the new layout. No `src-tauri/` source change needed because the `m2c-native` paths are still stubs. | 2026-06-16 |
| 165 | The `stt::tests::list_installed_models_*` test is split into two feature-gated tests: a stub-only test that asserts "every curated id is present" and a real-path test that asserts "empty on a fresh app data dir." | The two branches of `list_installed_models` (cfg-gated) have different return shapes (stub: every curated id, real: only what's on disk). With `m2c-native` now buildable, the previously-stub-only test had to be cfg-gated too, and a corresponding real-path test was added to pin the real-shape contract. | 2026-06-16 |
| 166 | The m2c-native build is "buildable" but the cpal + whisper inference wiring remains a separate follow-up. | Building whisper.cpp via CMake and the bindgen step is now proven end-to-end, but the `m2c-native` `start_listening` / `stop_listening` / `install_model` bodies are still stubs that just `Ok(())`. Wiring real mic capture (cpal, with platform stream configuration) and real whisper inference (the new `WhisperContext::new_with_params` + `WhisperState::full` API) is a one-day follow-up on a real machine. The current daily-driver STT is Web Speech, which is fully functional. | 2026-06-16 |
...
### 9.44 Phase 9.2f — SHIPPED (Bridge multi-client aggregator, see CHANGELOG "Added (Phase 9.2f — Bridge multi-client aggregator)")

The 9.2e handoff explicitly deferred the bridge multi-client aggregator to 9.2f. The motivation: a single Monaco pane with *multiple* open models of *different* kinds (the "polyglot workspace" case — 5 open tabs, 4 different kinds, 4 different LspClients all live at once). Pre-9.2f, the bridge keyed on the focused model's kind and re-ran the effect on every kind change. The 9.2f bridge tracks *all* open models, registers one provider set per kind, and dispatches traffic per-model to the right client.

**KIND_TO_LANGUAGE_IDS per-kind `DocumentSelector`.** The new `KIND_TO_LANGUAGE_IDS: Record<LspServerKind, readonly string[]>` constant in `lspClientStore.ts` is the per-kind `DocumentSelector` the bridge uses to register one provider set per kind. The mapping is: `typescript` → `['typescript', 'typescriptreact', 'javascript', 'javascriptreact']`; `rust_analyzer` → `['rust']`; `pyright` → `['python']`; `unknown` → `[]` (no provider). Monaco's provider registry routes provider calls to the right set per file based on the `DocumentSelector` match — no in-bridge router.

**Multi-model aggregator (bridge).** The new `useMonacoLspBridge` does:

1. **Discovery** on mount: walks `monaco.editor.getModels()` (the *global* Monaco model registry) and hooks every open model. The pre-9.2f bridge only knew about the focused model.
2. **Subscribe to future models**: `monaco.editor.onDidCreateModel` adds a new model when a tab is opened. The new model is hooked (per-model `onDidChangeContent` + `onWillDispose` subscriptions) and a `didOpen` is sent to the right kind's client.
3. **Per-model lifecycle**: each model's `onDidChangeContent` routes `didChange` to the right client (kind inferred from URI). Each model's `onWillDispose` routes `didClose` and tears down the per-model subscriptions.
4. **Per-kind provider sets**: registers one `registerLspProviders` call per *supported* kind (3 today: typescript, rust_analyzer, pyright), each with the kind's `DocumentSelector`. Pre-9.2f registered 1 set per effect run (re-run on kind change). 9.2f registers 3 sets at mount and Monaco's provider registry routes per file.
5. **Effect deps**: the effect no longer re-runs on `clientHandleId` change (the pre-9.2f re-key on respawn). The per-model `didChange` / `didClose` dispatch looks up the client lazily on every event, so a respawn is reflected without an effect re-run. Provider respawn (re-registering on a fresh `LspClient` after a crash + respawn) is deferred to a follow-up slice — the provider captures the client at registration time, so post-respawn `didChange` lands on the new client (the dispatch path) but the provider itself still points at the old one. A future slice can add respawn-aware re-registration by subscribing to the store's `clients` map for the kind's `handleId` change.
6. **Cleanup**: on bridge unmount, *every* per-model subscription is disposed and a `didClose` is sent for every open model; *every* per-kind provider set is disposed. The bridge is now forward-compatible with a `EditorPane` refactor that keeps a single Monaco instance across tab switches.

**`EditorPane` refactor deferred.** The 9.2e handoff's "next session should resume from Phase 9.2f" included a `EditorPane` refactor (drop `key={activeTab.id}`, persist Monaco across tab switches, switch model in place via `editor.setModel()`). That refactor is **deferred** to a follow-up slice. The aggregator is correct in both the current (per-tab Monaco) and the future (single-Monaco-across-tabs) designs — no bridge change needed when the pane refactor lands. The slice ships the structural infrastructure (the aggregator) so the pane refactor is a pure UI change.

**Test coverage (9.2f additions, +10 tests):**

- **5 new bridge aggregator tests** in `useMonacoLspBridge.test.tsx`:
  - `aggregator: 5 open tabs of 4 different kinds spawn 4 distinct LspClients` — the polyglot-workspace case (2x TS, 1x py, 1x rs, 1x tsx → 3 distinct LspClients because TS/TSX both map to `typescript`).
  - `aggregator: content changes on a .py model route to the pyright client, not the TS client` — verifies the per-model dispatch routes to the right client.
  - `aggregator: opening a new tab (onDidCreateModel) sends didOpen to the right client` — verifies the create-model subscription hooks new models.
  - `aggregator: closing a tab (onWillDispose) sends didClose to the right client` — verifies the per-model dispose subscription fires didClose.
  - `aggregator: per-kind kill switch off for pyright — .py model is not opened on a client` — verifies the per-kind kill switch still gates the multi-model aggregator.

- **5 new `KIND_TO_LANGUAGE_IDS` tests** in `lspClientStore.inferServerKind.test.ts`:
  - Guard the per-kind `DocumentSelector` mapping. Future slice that adds a new supported kind but forgets to fill in the `KIND_TO_LANGUAGE_IDS` entry will fail this test.

- **Test fixture updates** in `useMonacoLspBridge.test.tsx`:
  - New `fakeGetModelsReturn` + `fakeCreateModelListeners` for the bridge to discover existing models and subscribe to future creations.
  - New `makeFakeModel` factory wires up the per-model `onDidChangeContent` and `onWillDispose` subscriptions the bridge installs.
  - All 15 pre-existing tests updated to use the new fixture (per-model events instead of editor-level events; 3 `registerLspProviders` calls instead of 1).

- **2 pre-existing tests removed**:
  - `sends didClose for the old model and didOpen for the new one on file switch` — replaced with two 9.2f tests (`sends didOpen for a new model created via onDidCreateModel` + `sends didClose when a model is disposed via onWillDispose`) that test the new lifecycle hooks (which are the forward-compatible mechanism; the pre-9.2f editor-level `onDidChangeModel` was a per-Monaco-instance API that doesn't exist in the future single-Monaco-across-tabs design).

**Design decisions:**

| # | Decision | Rationale |
|---|----------|-----------|
| 143 | The bridge tracks *all* open Monaco models, not just the focused one. | The pre-9.2f design keyed the effect on `(editor, workspaceRoot, clientHandleId)` and re-ran on every kind change. The 9.2f design keys on `(editor, workspaceRoot)` and tracks models internally. The hook now has no per-kind dep; per-model state is maintained in a `Map<modelUri, ModelState>` for the bridge's lifetime. Effect cleanup is comprehensive (every per-model subscription disposed; every per-kind provider set disposed; `didClose` sent for every open model). |
| 144 | One provider set per kind, registered at bridge mount. | The 9.2f bridge iterates `SUPPORTED_LSP_SERVER_KINDS` once at mount and calls `registerLspProviders` once per enabled kind, each with the kind's `DocumentSelector`. Monaco's provider registry routes per file based on the `DocumentSelector` match — no in-bridge router. The 3 provider sets coexist in Monaco's registry for the lifetime of the bridge. The `includeCompletion: boolean` option is passed to each kind's provider set based on the global Phase 9.6 completion sub-toggle. |
| 145 | The `EditorPane` refactor (drop `key={activeTab.id}`, persist Monaco across tab switches) is **deferred** to a follow-up slice. | The aggregator is correct in both the current (per-tab Monaco, the bridge sees one model at a time) and the future (single-Monaco-across-tabs, the bridge sees N models) designs — no bridge change needed when the pane refactor lands. The pane refactor is a *UI* concern (whether the editor persists across tabs); the bridge is a *seam* concern (how Monaco models map to LSP clients). The 9.2f slice ships the seam. |
| 146 | Provider respawn (re-registering on a fresh `LspClient` after a crash + respawn) is deferred. | The provider captures the client at registration time (its closures call `client.request(...)` against the captured reference). The `didChange` / `didClose` dispatch path looks up the client lazily on every event, so post-respawn `didChange` lands on the new client (the dispatch path is respawn-aware). The provider itself still points at the old client; provider respawn handling is deferred to a follow-up slice. A future slice can add explicit respawn-aware re-registration by subscribing to the store's `clients` map for the kind's `handleId` change and disposing + re-registering the provider set on respawn. The current behavior is "good enough" for the 9.2f MVP: the user gets cross-file go-to-def / find-references / etc. via the *old* client (which is now dead — the server has been respawned). The follow-up slice fixes the respawn edge case. |

### 9.45 Phase 9.30a — SHIPPED (`lsp_check_available` per-kind dispatch + `kindToSpawnSpec` parity, see CHANGELOG "Added (Phase 9.30a — `lsp_check_available` per-kind dispatch + `kindToSpawnSpec` parity)")

Phase 9.2 declared the `LspServerKind` taxonomy in the bridge / store, but the *check* IPC (`lsp_check_available`) was still hard-coded to `typescript-language-server` on both sides. Phase 9.30a makes the check call per-kind and pins the JS spawn spec ↔ Rust check table parity in a test, so the two sides of the wire can never drift.

**Rust side — `src-tauri/src/stdio.rs` + `lib.rs`.** New `LspServerKind` enum (snake_case serde: `typescript`, `rust_analyzer`, `pyright`, `unknown`) + new `CheckAvailableArgs { server_kind: Option<LspServerKind> }` struct (camelCase serde on the wire, so the JS side passes `{ serverKind: 'rust_analyzer' }`). The new `server_kind_spec(kind)` private helper returns the per-kind `(binary, install_hint)` tuple, and the new generic `check_available_for(spec)` helper runs the existing PATH-probe + `--version` flow against any binary. `check_available()` is now a dispatcher: pick the kind (defaulting to `Typescript` when `args.server_kind` is `None`, preserving the pre-9.30a behaviour), look up the spec, delegate. The `Unknown` arm returns `available: false, install_hint: ""` (the bridge shouldn't call us, but the dispatcher is total). `lib.rs` updates `lsp_check_available` to take an `Option<CheckAvailableArgs>` and re-exports the new types.

**TS side — `src/ipc/lsp.ts`.** New `LspServerKind` union (4 literals — matches the Rust enum), new `CheckAvailableArgs` interface, new `LspSpawnSpec` interface (`{ command, args, installHint }`), new pure `kindToSpawnSpec(kind)` function (the canonical JS-side spawn spec), and the `lspCheckAvailable(args?)` wrapper now forwards the args through to the Tauri command unchanged. The JS side is the source of truth for the *spawn* spec (the `LspClient` calls `lspRunStdio({ command, args, cwd })` — the `command` + `args` come from `kindToSpawnSpec(kind)`). The Rust side is the source of truth for the *check / install-hint* spec (the settings card calls `lspCheckAvailable({ serverKind })` and surfaces the `install_hint` from the result). They agree on the binary name; the new `lsp.test.ts` pins that contract.

**Why the split?** The Rust `lsp_run_stdio` command is *kind-agnostic* — it spawns whatever the JS side passes in `command` + `args`. Only the `lsp_check_available` dispatch needs to know about kinds (so the right `install_hint` comes back). So the JS side owns the spawn spec and the Rust side owns the check spec; the contract between them is "binary name + install hint agree" and the test pins it.

**Test coverage (10 new tests in `src/ipc/lsp.test.ts`):**

- **5 `kindToSpawnSpec` tests** — canonical spec for `'typescript'`, `'rust_analyzer'`, `'pyright'`; empty spec for `'unknown'`; and a cross-side parity test that asserts the JS `spec.command` + `spec.installHint` agree with the Rust `server_kind_spec(kind).binary` + `spec.install_hint` for every known kind.
- **5 `lspCheckAvailable` tests** — invokes the Tauri command with `args: undefined` when called with no args; forwards an explicit `{ serverKind: 'rust_analyzer' }` / `{ serverKind: 'typescript' }` / `{ serverKind: 'pyright' }` / `{ serverKind: 'unknown' }` unchanged.

**Design decisions:**

| # | Decision | Rationale |
|---|----------|-----------|
| 147 | The JS `kindToSpawnSpec()` and the Rust `server_kind_spec()` are *two* tables that must agree on binary name + install hint. | The cross-side test (`returns specs whose binary names match the Rust server_kind_spec table`) pins that contract. Adding a new kind is a 1-arm match change in both files + 1 new row in the test's `cases` table. The `LspClient` reads `kindToSpawnSpec` at spawn time; the settings card reads the result of `lspCheckAvailable` for the install hint. If the JS side adds a kind without a Rust counterpart (or vice versa), the cross-side test catches it. |
| 148 | `lspCheckAvailable` is total over the full `LspServerKind` union (including `'unknown'`). | The bridge is still expected to gate on `isSupportedKind` *before* calling `lspCheckAvailable` (otherwise it's wasted IPC), but the IPC layer doesn't enforce that. The Rust side returns `available: false, install_hint: ""` for `'unknown'`; the JS wrapper is a pass-through. The total-return design keeps the type system honest — the function signature doesn't have an "invalid kind" branch to handle. |

### 9.46 D-145 — SHIPPED (single Monaco instance across tab switches, see CHANGELOG "Added (Phase 9.45 — D-145: single Monaco instance across tab switches)")

Phase 9.2f's bridge aggregator was forward-compatible with both a "per-tab Monaco" and a "single Monaco across tabs" `EditorPane` design — the aggregator tracks models globally via `monaco.editor.getModels()` + `onDidCreateModel`, so it doesn't care which design the pane uses. D-145 flips the pane to the single-Monaco design, which the 9.2f handoff explicitly deferred as "a pure UI change."

**The pre-D-145 design.** `<ActiveEditor>` was keyed on `key={activeTab.id}`, so the Monaco instance was destroyed and remounted on every tab switch. That was correct but expensive:
1. **Controller-store flash.** `useMonacoLspBridge` and `useInlineEditOverlay` briefly saw `editor: null` between unmount + remount. `CmdKModal` lost its handle for the duration of the flash.
2. **Bridge aggregator re-run.** The 9.2f aggregator's effect re-ran on every tab switch (because `editor` was a new reference). It re-discovered all open models via `monaco.editor.getModels()`, re-registered per-kind provider sets, and re-subscribed to per-model events. The cost was small (the bridge is fast) but the *user-visible* effect was a momentary gap in cross-file go-to-def / find-references after every tab click.
3. **Inline-edit overlay re-mount.** `useInlineEditOverlay` tore down + re-mounted its content widget, decoration collection, and Tab/Esc keybindings on every tab switch.
4. **View state re-computation.** The library re-saved + re-restored the view state (scroll / selection / undo stack) on every remount, even though the user was switching between tabs they'd already opened (the view state was already saved).

**The D-145 design.** Drop the `key={activeTab.id}`. The `@monaco-editor/react` `Editor` component handles the model swap in place via the `path` prop: when `path` changes, the library looks up the existing model by URI, calls `editor.setModel(model)`, and restores the saved view state automatically. No React remount, no effect re-run, no store flash. The 9.2f aggregator stays mounted for the pane's lifetime — its `useEffect` doesn't re-run on tab switch because `editor` is the same reference.

**Lifecycle fixes.** Three changes were needed beyond just dropping the `key`:

1. **`[content]` effect is now path-aware.** Pre-D-145, the effect's `setValue(content)` call was how the pane pushed the new tab's content into the *fresh* Monaco instance on every tab switch. With the persistent editor, the `<Editor>` library's internal `useUpdate` on `path` change handles the model swap (which carries the new tab's content). The effect's `setValue` would now race the library's `setModel`. The fix: read `editorRef.current.getModel()?.uri.path` and early-return if it doesn't match the active `path`. The `setValue` branch is now reserved for "external content update on the same tab" (file-watcher reload, AI panel apply). (D-149)

2. **`pendingReveal` moved to `onDidChangeModel` subscription.** Pre-D-145, the reveal check lived inside `handleMount` (which fired on every tab switch because `ActiveEditor` remounted). With the persistent editor, `handleMount` fires only once (on first mount), so the reveal would never fire on tab switch. The fix: subscribe to `editor.onDidChangeModel` and run the reveal check whenever Monaco swaps models. The `currentPath` check (with the new model URI's path) is robust to the order of `useUpdate` effects. (D-150)

3. **`setControllerEditor(null)` cleanup is now real-unmount-only.** Pre-D-145, the cleanup fired on every tab switch (because `ActiveEditor` remounted). The `setControllerEditor(null)` call briefly flashed the store to `null` between unmount + remount. With the persistent editor, `ActiveEditor` only unmounts when the whole `EditorPane` unmounts (screen navigation), so the cleanup runs only in the legitimate "user navigates away from the editor" case. Pre-D-145 the effect was the *cause* of the controller flash; D-145 makes it the *fix*.

**Test coverage (5 new tests in `EditorPane.test.tsx`):**

- `writes the editorControllerStore on first mount with the active tab path` — verifies the controller store gets the editor handle on first mount (the mock `<Editor>`'s `onMount` callback writes the fake editor to the store).
- `forwards the new tab path to the <Editor> on tab switch (D-145 in-place model swap)` — verifies the `<Editor>` is the same React element across tab switches (the `path` prop flows through as a prop change, not a remount). Pre-D-145, the `<Editor>` would have been remounted on tab switch, and the new path would have been passed to a *fresh* component instance.
- `does NOT clear the editorControllerStore on tab switch (D-145 — no controller flash)` — the contract the pre-D-145 design violated. The store is written once on first mount and stays stable across tab switches.
- `clears the editorControllerStore on real unmount (D-145 still handles the screen-navigation case)` — the cleanup still works for the legitimate "user navigates away from the editor" case. Pre-D-145, the cleanup was over-eager (fired on tab switch); D-145 narrows it to real unmount.
- `does not re-run the LSP bridge effect on tab switch (D-145 — bridge stays mounted)` — the bridge aggregator (shipped in 9.2f) is now forward-compatible with the single-Monaco design. The mock counts `useMonacoLspBridge` mount cycles via `useEffect` (a function-body counter would count renders, not mounts); the count is captured before the first tab switch and re-checked after, asserting the delta is zero.

**Test infra.** The test mocks `@monaco-editor/react`'s `Editor` component as a function that captures the `path` prop and simulates the `onMount` callback. The mock is a no-op render (returns `null`) — the test asserts on the captured props and on the controller store, not on a real Monaco tree. `monaco-editor` is mocked because the real ESM package doesn't load in jsdom and `EditorPane` touches `monaco.languages.typescript.typescriptDefaults` at module init time (the `configureTsServiceOnce` guard makes it idempotent but it still imports the module).

**Design decisions:**

| # | Decision | Rationale |
|---|----------|-----------|
| 149 | The `[content]` effect is path-aware (reads `editorRef.current.getModel()?.uri.path` and skips `setValue` if it doesn't match the active `path`). | This is the cleanest separation: the `<Editor>` library owns the "tab switch" path (its internal `useUpdate` on `path` change calls `setModel(model)`), the `[content]` effect owns the "external content update" path, and the two are non-overlapping. The `setValue` branch is reserved for the cases where the active tab is the same but its content changed externally (file-watcher reload, AI panel apply). |
| 150 | The `pendingReveal` check moved from `handleMount` to a `useEffect` that subscribes to `editor.onDidChangeModel`. | The pre-D-145 design relied on `handleMount` firing on every tab switch (because the `<Editor>` remounted). With the persistent editor, `handleMount` fires only once, so the reveal would never fire. `onDidChangeModel` is the correct event for "model just swapped" — it fires after the library's internal `useUpdate` on `path` has run, so the `currentPath` check sees the new model's URI, not the previous tab's. The subscription is disposed on pane unmount (the `useEffect` cleanup). |
| 151 | The 9.2f bridge aggregator's "no re-run on tab switch" property is now actually exercised. | Pre-D-145, the bridge re-ran on every tab switch (because the editor instance was a new reference). D-145 makes that re-run unnecessary — the bridge subscribes globally to `monaco.editor.getModels()` + `onDidCreateModel` + per-model events, and the editor instance is now stable across tab switches, so the aggregator's effect deps (`[editor, workspaceRoot]`) don't change. The 5th new test pins this contract: the bridge mock's mount counter doesn't increment on tab switch. |

### 9.47 D-146 — SHIPPED (LSP provider re-registration on respawn, see CHANGELOG "Added (Phase 9.47 — D-146: LSP provider re-registration on respawn)")

Phase 9.2f's bridge aggregator is *respawn-aware on the dispatch path* — the per-model `didChange` / `didClose` dispatch looks up the `LspClient` lazily on every event, so a crashed + respawned client is used correctly for document-state sync. But the *providers* — the closures that capture `client` for `client.request('textDocument/definition', ...)`, `client.request('textDocument/hover', ...)`, etc. — were registered once at bridge mount and pointed at the client from that mount. After a respawn, the new `LspClient` was in the store with a new `handleId`; the dispatch path used it correctly, but **go-to-def / hover / find-references / rename / code actions silently failed** until the bridge re-mounted (e.g. the user toggled the kill switch, or reloaded the workspace). D-146 closes that gap.

**The pre-D-146 design.** `registerProvidersForKind(kind)` was called once per kind from the main `useEffect`, captured the `LspClient` instance from the synchronous `getOrCreate`, and registered the kind's provider set against that client. The provider closures (`registerHoverProvider(client, ...)`, etc.) baked in the `client` reference. After a respawn, the store's `lspClientStore` replaced the client (new `handleId`, new `LspClient` instance), but the providers still held the dead reference. The `client.request(...)` call would hit the dead `LspClient`'s `_pending` map, which had been cleared on `dispose()`, so the request would hang (and eventually time out, surfacing as a Monaco "No definition found" / spinner).

**The D-146 design.** Subscribe the bridge to `lspClientStore`'s `clients` map; on any `handleId` change for a supported kind, **dispose the old provider set and re-register a fresh one against the new client.** The respawn itself is owned by the store's Phase 9.5 backoff ladder; the bridge only reacts to the observable `handleId` change. The flow is:

1. The store's `respawn` (or auto-respawn after a crash) calls `dispose` + `getOrCreate`. The new client has a fresh `handleId`. The store's `setState` updates the `clients` map.
2. The bridge's `useLspClientStore.subscribe(onStoreChange)` fires with the new state.
3. The subscriber iterates supported kinds. For each kind, it reads `state.clients.get(workspaceKindKey)` and compares the current `handleId` to `lastHandleIdsByKind.get(kind)`. A mismatch signals a respawn.
4. **Optimistically** update `lastHandleIdsByKind` with the new `handleId` (so the cascade of `setState`s from `getOrCreate` doesn't re-trigger the subscriber).
5. Call `void registerProvidersForKind(kind)`. The function disposes the kind's existing provider set (from `providerDisposables`), then `await`s a fresh `getOrCreate` (which returns the already-running new client — no new spawn), then registers a fresh provider set.
6. The async re-registration eventually re-sets `lastHandleIdsByKind` (with the same value) — idempotent.

The subscription is torn down in the `useEffect` cleanup (`unsubStore()`) to prevent leaks and post-navigation ghost re-registrations.

**Why optimistic update matters (D-146.1).** `getOrCreate` fires multiple `setState`s (client creation, status `starting`, status `ready`). Without the optimistic update, each of those `setState`s would retrigger the subscriber (because the first observation of the new `handleId` happens early, but the actual `setState`s fire later), and the redundant async re-registrations would race. The optimistic update *before* the `void registerProvidersForKind` call breaks the loop: the subscriber sees the matching `handleId` on subsequent `setState`s and skips. The eventual `registerProvidersForKind` does its work, the final `setState` (after re-registration) re-sets `lastHandleIdsByKind` to the same value — idempotent.

**Why the first-observation guard matters (D-146.2).** The subscriber fires during the initial mount's `setState` chain (when `getOrCreate` first creates the 3 clients). Without the guard, the bridge would call `registerProvidersForKind` twice per kind — once via the synchronous call inside the effect, once via the subscription on the first observation. The guard (`if (lastHandleId === undefined) continue`) skips the first observation per kind. Subsequent observations (after the first re-registration) populate `lastHandleIdsByKind`, and from then on the subscriber only fires on real `handleId` changes.

**`registerProvidersForKind` extraction.** The provider-registration logic was inlined in the main effect. Extracting it into a named async function makes it safe to call multiple times for the same kind (the function disposes existing disposables first, then registers a fresh set). It also makes the respawn subscription trivial to wire up: `void registerProvidersForKind(kind)` from the subscriber. The function checks `cancelled` at multiple points (after the `await getOrCreate`) so an unmount mid-respawn doesn't leak the new provider set.

**Test coverage (3 new tests in `useMonacoLspBridge.test.tsx`):**

- `D-146: re-registers providers after a respawn (new handleId)` — calls `respawn('/workspace/respawn', 'typescript')` after the initial mount, asserts that `registerLspProviders` is called exactly once *more* (for the respawned kind), and the other 2 kinds don't get a redundant call. Pins the "only the respawned kind triggers re-registration" contract.
- `D-146: disposes the old provider set on respawn (not the new one)` — captures the `disposables` arrays returned by each `registerLspProviders` call, asserts the *old* set's `dispose()` is called on the kind's respawn, while the *new* set's disposables stay live. Pins the "old set torn down, new set live" contract.
- `D-146: unsubscribes the respawn watcher on bridge unmount` — captures the `unsubStore` callback returned by `useLspClientStore.subscribe`, asserts it's called in the bridge's `useEffect` cleanup. Pins the "no ghost re-registrations after navigation" contract.

**Test infra fix (uncovered by the new tests).** The test mock's `lspStdioRead` was a single global FIFO (`__lspQ`). After a respawn, the new client's `initialize` response could be consumed by a still-alive sibling client's reader loop, which dispatched the message to a `_pending` map with a different `id` and left the new client hanging. D-146's respawn test exposed this. The mock now uses a per-`handleId` queue (`__lspQByHandle: Map<handleId, Uint8Array[]>`), mirroring how the Rust side delivers each handle's stdout independently. This is a **test-only** change with no production impact — it just makes the mock accurately model the per-handle stdout delivery that the Rust `stdio.rs` provides. The pre-existing 21 tests in this file continue to pass with the per-handle queue (the 3 initial clients each get their own queue, responses don't cross-contaminate).

**Design decisions:**

| # | Decision | Rationale |
|---|----------|-----------|
| 152 | The bridge subscribes to `lspClientStore`'s `clients` map and reacts to `handleId` changes per kind. | The store is the single source of truth for `LspClient` lifecycle. The bridge is a consumer; it shouldn't track respawns independently. The observable `handleId` change is the correct signal — it's set by the store's `setState` in `respawn` and in `getOrCreate` (after the new client is created). The bridge doesn't need to know about backoff, crash detection, or kill-switch logic. |
| 153 | Optimistic `lastHandleIdsByKind` update *before* the async re-registration. | `getOrCreate` fires a cascade of `setState`s (client creation, status transitions). Without optimistic update, the subscriber fires on each, sees the "new" `handleId` for the first time, and re-triggers re-registration. The redundant async re-registrations race and leave provider disposables in a torn state. Optimistic update breaks the loop without losing the re-registration. |
| 154 | First-observation guard (`if (lastHandleId === undefined) continue`). | The subscriber fires during the initial mount's `setState` chain. Without the guard, the bridge would call `registerProvidersForKind` twice per kind (once synchronously, once via the subscriber). The guard skips the first observation; subsequent observations (after the first re-registration) populate `lastHandleIdsByKind`. |
| 155 | Per-handleId queue in the test mock (`__lspQByHandle` instead of `__lspQ`). | D-146's respawn test exposed a race in the mock's `lspStdioRead`: a single global FIFO meant the new client's `initialize` response could be consumed by a still-alive sibling's reader. The mock now mirrors the Rust side's per-handle stdout delivery. This is test-only — production code is unaffected. |
| 156 | `registerProvidersForKind` is an async function with `cancelled` checks at multiple points. | The function `await`s `getOrCreate`, which means the bridge could unmount mid-respawn. Without the `cancelled` checks, the post-await code would run on an unmounted component, register provider disposables that never get cleaned up (the cleanup runs on unmount, but the new disposables wouldn't be in the `providerDisposables` map yet), and leak Monaco listeners. The `cancelled` checks at the entry and after the `await` ensure the function short-circuits if the bridge is gone. |

### 9.48 Phase 7.1.1 / 7.1.2 — SHIPPED (JSON / CSS / HTML default-service configuration + worker-routing tests, see CHANGELOG "Added (Phase 7.1.1 / 7.1.2 — JSON / CSS / HTML default-service configuration + worker-routing tests)")

Phase 7 (shipped earlier this session) wired up Monaco's language-service Web Workers (TypeScript, JSON, CSS, HTML) via `getMonacoWorker.ts` — five Vite `?worker` imports, a `MonacoEnvironment.getWorker` resolver, and matching `optimizeDeps.include` + `manualChunks` in `vite.config.ts`. But it left the **`jsonDefaults` / `cssDefaults` / `htmlDefaults` configuration objects at Monaco's hard-coded factory values**, which produced three real-world behaviours that didn't match the project's conventions:

1. **JSON**: Monaco's default treats `//` comments as **errors** (per RFC 8259) and trailing commas as errors. Real-world `tsconfig.json` / `package.json` / `.eslintrc.json` files routinely have both, and the result is a sea of red squiggles on every config file users open.
2. **CSS**: Monaco's lint rules flag `vendorPrefix` and `compatibleVendorPrefixes` as **warnings** by default, producing noise on any stylesheet that uses `-webkit-` / `-moz-` prefixes (which is most production CSS). `idSelector` and `important` are similarly noisy.
3. **HTML**: Monaco's HTML defaults are fine — but the `setOptions` API requires *every* field of `HTMLFormatConfiguration` (no partial updates), so we don't call it.

7.1.2 also pins the worker routing itself. Before this slice, the `?worker` imports and the label switch in `getMonacoWorker.ts` were **untested** — a "rename a label" change would silently mis-route workers. The new 12-test suite verifies the right worker class is returned for each label, the `typescript` / `javascript` shared-service behaviour, and the fresh-instance-per-call contract.

**The pre-7.1.1 design.** `getMonacoWorker.ts` only handled *worker registration* (which worker class to instantiate for which label). The defaults objects — `monaco.languages.json.jsonDefaults`, `monaco.languages.css.cssDefaults`, `monaco.languages.html.htmlDefaults` — were never touched after Monaco's module init. The defaults Monaco ships with are RFC-strict (no comments, no trailing commas) and lint-strict (vendor-prefix warnings, `!important` warnings). The TS defaults were the only ones configured (via `configureTsServiceOnce` in `EditorPane.tsx`).

**The 7.1.1 design.** A new module `configureLanguageServices.ts` with a single exported `configureLanguageServices()` function that:

- Calls `monaco.languages.json.jsonDefaults.setDiagnosticsOptions({ validate: true, allowComments: true, trailingCommas: 'warning', enableSchemaRequest: false, schemas: [] })`. The `schemas: []` disables Monaco's bundled `http://json.schemastore.org/*` defaults (which would otherwise fetch from the network). The `enableSchemaRequest: false` prevents a `fetch()` on every `.json` file open.
- Calls `monaco.languages.css.cssDefaults.setOptions({ validate: true, lint: { compatibleVendorPrefixes: 'ignore', vendorPrefix: 'ignore', idSelector: 'ignore', important: 'ignore', zeroUnits: 'ignore' } })`. The `validate: true` keeps syntax checking on; the `lint: { ... }` block silences the rules that don't have an objective answer in a normal project.

`EditorPane.handleMount` calls `configureLanguageServices()` next to `configureTsServiceOnce()`. Both have module-level idempotency guards (matching the `tsServiceConfigured` pattern); both configure global Monaco state; both only need to run on the first mount. The pattern is exactly parallel.

**Why HTML is not configured (D-157).** Monaco's `HTMLFormatConfiguration` is **all-required** — you can't pass a partial object, you have to enumerate all 10+ fields. The defaults already match the editor's `tabSize: 2` + `insertSpaces: true`, so configuring HTML would mean re-stating the defaults verbatim just to add one or two customisations. We chose to skip it for now. If a future slice needs to pin `wrapAttributes: 'auto'` or similar, it has to enumerate the full config object — the `--- HTML ---` block in `configureLanguageServices.ts` documents this Monaco API quirk.

**Why `enableSchemaRequest: false` (D-158).** Monaco's `enableSchemaRequest: true` triggers a `fetch()` to the URL in a file's `$schema` field. Lipi is offline-first (Tauri app, no CDN calls), and a network schema fetch on every `.json` file open would be a serious regression. Users who want schema validation can register an explicit schema via a future schema store — that's a separate slice, not this one.

**Why 7.1.2 tests are mock-heavy.** The real `getMonacoWorker.ts` uses Vite's `?worker` import suffix (`import tsWorker from 'monaco-editor/esm/vs/language/typescript/ts.worker?worker'`). Vitest's import-analysis pipeline can't resolve this — Vite's `?worker` is a Vite-specific virtual-module syntax that Vitest's transform doesn't understand. The standard test pattern is to `vi.mock('./getMonacoWorker', () => ({}))` and re-implement the side-effect in a `vi.hoisted(...)` callback. The mock re-implements the *exact same routing logic* as the real module (typescript / javascript → TS worker, json → JSON worker, css / scss / less → CSS worker, html / handlebars / razor → HTML worker, default → editor worker). A future change to the real resolver that diverges from the mock will fail the tests, which is the correct failure mode.

**Why the `vi.hoisted(...)` pattern.** The mock has to install `MonacoEnvironment` *before* the test file's first `it()` block runs. `vi.hoisted(...)` runs the callback during Vite's transform phase, before any imports. The classes (`HoistedTsWorker`, etc.) are defined inside the callback so the closure is self-contained; the test scope reaches them via `globalThis.__monacoWorkerTs` etc.

**Test coverage (15 new tests across 2 files):**

`configureLanguageServices.test.ts` (3 tests):
- `first call: configures JSON and CSS defaults with the project conventions` — captures the full options object passed to each setter and asserts on the field-by-field shape (validate, allowComments, trailingCommas, enableSchemaRequest, schemas, lint rules). The first test in the file is the one that *spends* the idempotency guard.
- `subsequent calls are no-ops (module-level idempotency guard)` — calls `configureLanguageServices()` three more times and asserts the captured call counts don't increment. Pins the "configure-once" contract.
- `does NOT configure htmlDefaults (HTML format is all-required; defaults match the editor)` — implicit negative test: the mock factory doesn't even define an `html` namespace under `monaco.languages`, so any call to `monaco.languages.html.htmlDefaults.setOptions` would throw `Cannot read properties of undefined`. The "no-throw on subsequent calls" assertion in the previous test is the proof.

`getMonacoWorker.test.ts` (12 tests):
- `installs MonacoEnvironment.getWorker on the global scope` — sanity check that the hoisted callback ran.
- `routes 'typescript' label to the TS worker` — basic routing.
- `routes 'javascript' label to the TS worker (shared service)` — pins the documented shared-service contract so a "split them up" change shows up as a clear diff.
- `routes 'json' label to the JSON worker`.
- `it.each [['css'], ['scss'], ['less']]` — CSS family all share the CSS worker.
- `it.each [['html'], ['handlebars'], ['razor']]` — HTML family all share the HTML worker.
- `routes unknown labels to the base editor worker` — verifies the default branch with plaintext, markdown, xml, yaml, empty string, and a garbage label. Monaco's tokenizer / highlighter is enough for these; the editor worker provides it.
- `returns a fresh Worker instance on each call (no shared instance)` — pins the per-call `new TsWorker()`. A "cache the instance" change would break multi-workspace / multi-language scenarios.

**Design decisions:**

| # | Decision | Rationale |
|---|----------|-----------|
| 157 | HTML defaults are *not* configured. Monaco's `HTMLFormatConfiguration` is **all-required** (no partial updates) and the defaults already match the editor's `tabSize: 2` + `insertSpaces: true`. | The Monaco API asymmetry between JSON / CSS (partial `setOptions` / `setDiagnosticsOptions`) and HTML (all-required) makes "pin a single field" impossible without enumerating the full shape. The cost of doing it (verbose 10+-field config object) outweighs the benefit (matching the editor's already-matching defaults). The `--- HTML ---` block in `configureLanguageServices.ts` documents this for the next agent. If a future slice needs to pin `wrapAttributes: 'auto'` or similar, it has to enumerate the full config. |
| 158 | `enableSchemaRequest: false` (and `schemas: []` for the bundled schemastore.org defaults). | Monaco's `enableSchemaRequest: true` triggers a `fetch()` to the URL in a file's `$schema` field. Lipi is offline-first (Tauri app, no CDN calls), and a network schema fetch on every `.json` file open would be a serious regression. The bundled schemastore.org schemas are also network-dependent. Users who want schema validation can register an explicit schema via a future schema store — that's a separate slice. |
| 159 | The `getMonacoWorker.test.ts` mock re-implements the resolver in a `vi.hoisted(...)` callback. The real module can't be imported in tests because it uses Vite's `?worker` import suffix, which Vitest's import-analysis pipeline can't resolve. | The standard test pattern for `?worker` / `?url` / `?raw` imports is to `vi.mock(...)` the source module and re-implement its side-effect. The mock mirrors the real routing logic exactly; a future divergence between the real module and the mock is the correct failure mode. The file-level doc comment in `getMonacoWorker.test.ts` documents this for the next agent. |
| 160 | `configureLanguageServices()` is called from `EditorPane.handleMount` next to `configureTsServiceOnce()`, not from a side-effect import in `main.tsx`. | Both have module-level idempotency guards. Both configure global Monaco state. Both only need to run on the first mount. Putting them in the same place makes the "first-mount-only" contract obvious — a single `handleMount` call configures everything. The TS version lives in `EditorPane.tsx` because it reads `compilerOptions` from `tsConfigStore`; the JSON / CSS version lives in its own module because it has no editor-local state. |

---

### 9.46 Phase M6c — SHIPPED (per-tab cursor + file-tree scroll, see CHANGELOG "Phase M6c")

**Preserves the M6b architecture from §9.23 — Decisions #81 ("all per-tab state is in `WorkspaceTabState`"), #83 (one-way mirror-back from live store to persisted state), and #84 (`setTabState` equality short-circuit) are still authoritative. M6c is a strict extension; the M6b non-goals list (per-tab font / theme / recents / tool / voice / git / search settings, see `docs/plans/m6b-design.md`) is unchanged.**

The M6b design (`docs/plans/m6b-design.md`) explicitly parked scroll and cursor memory as out-of-scope — Decision #81's "all per-tab state is in `WorkspaceTabState`" invariant was preserved, but the per-file cursor and per-tab file-tree scroll position were left for a follow-up. M6c is that follow-up: the minimal slice that picks up the two scroll/cursor items, leaving the M6b non-goals list authoritative for a later phase.

**Data model (Decision #167).** `WorkspaceTabState` is extended with `editorCursorByPath: Record<filePath, { line, column }>` and `fileTreeScrollAnchor: string | null` (the topmost visible row's `data-tree-path`). The v3 → v4 → v5 in-memory migration chain synthesises the 2 new fields with empty defaults; users on v3 / v4 files do not see any per-file cursor or scroll data in the upgraded export.

**Mirror-back architecture.** Two `useEffect`s per owning component. `EditorPane.tsx`'s `ActiveEditor` subscribes to `editor.onDidChangeCursorPosition` (debounced via `requestIdleCallback` / `setTimeout(500ms)` fallback in `scheduleCursorMirrorBack`, Decision #168) and writes to `useWorkspaceStore.setEditorCursor`; the same component rehydrates the saved cursor via `editor.setPosition` + `revealPositionInCenterIfOutsideViewport` on `onDidChangeModel`, guarded by a `suppressNextCursorChange` ref to prevent the rehydrate itself from triggering a mirror-back round-trip. `FileTreePane.tsx`'s `TreeRoot` does the symmetric thing: rows carry `data-tree-path={entry.path}`, the rehydrate `useEffect` waits one rAF (with up to 2 retries for async `entriesByDir` loads) and calls `row.scrollIntoView({ block: 'start' })`, the mirror-back `useEffect` attaches a passive `scroll` listener throttled to rAF, finds the first row whose bottom is below the container top + 1px, and writes the path with a transition-only write guard. Both effects unmount-flush synchronously so the user's last position is always persisted.

**Settings format (Decisions #170 + #171).** The export/import format bumps from v4 to v5 (`src/shared/settingsIOv5.ts` + the matching `apply` and `preview` modules). v4 → v5 in-memory migration synthesises the 2 new fields with empty defaults; v3 → v4 → v5 chains reuse the v4 module's helpers. The `PrivacyDataCard` now exports in v5 format and shows the user a notice that v3 / v4 imports are upgraded on load (with empty per-file cursor / scroll). The editor cursor's `setEditorCursor` action has an equality short-circuit to avoid no-op writes; the file-tree scroll mirror-back has a transition-only write guard to avoid `null`-storms on an empty tree (both per Decision #171, the M6b loop-guard pattern preserved for the 2 new fields).

**Decisions (M6c):**

| # | Title | Rationale | Risks / follow-ups |
|---|-------|-----------|-------------------|
| 167 | `WorkspaceTabState` is extended with `editorCursorByPath` + `fileTreeScrollAnchor`. | Decision #81's invariant ("all per-tab state is in `WorkspaceTabState`") is preserved. The 2 new fields are the minimal slice that picks up the M6b non-goals. | The v3 / v4 → v5 migration synthesises empty defaults; users on v3 / v4 lose any per-file cursor + per-tab scroll they had. Acceptable because the data didn't exist in those formats. |
| 168 | The editor cursor mirror-back is throttled with `requestIdleCallback` (500ms timeout) or `setTimeout(500ms)` in test envs. A trailing-debounce per `tabId+filePath` ensures the store only sees a write after the user pauses typing. The flush-on-unmount is synchronous so a cursor move made just before a tab close is never lost. | `requestIdleCallback` is the right primitive (write happens on idle, not on a fixed schedule); the `setTimeout` fallback is for jsdom (no `requestIdleCallback`); the synchronous flush is for the unmount cleanup to guarantee the user's last position is persisted. | None — the throttle is well-isolated, the fallback is tested, and the flush is exercised by the `unmount flushes the pending cursor` test. |
| 169 | The file-tree scroll anchor is the topmost-visible *path* (not a pixel offset). The rehydrate scrolls the matching row into the topmost slot via `scrollIntoView({block: 'start'})`, which is layout-agnostic. Stale anchors (path no longer in the tree) silently fall back; the anchor is not auto-pruned on read. | The path-based anchor survives different display scaling / row height settings and works on any machine. `data-tree-path={entry.path}` is the DOM marker; the selector uses `CSS.escape(anchor)` to safely embed paths that may contain `.` or `\\`. | jsdom doesn't implement `CSS.escape`; the polyfill lives in `vitest.setup.ts` (not in production code). Production is unaffected. |
...
5. `docs/store-metadata/app-store.md` + `docs/store-metadata/google-play.md` + `docs/store-metadata/README.md` — store-metadata templates. Pre-fill the privacy-nutrition-label / data-safety-form (the non-obvious part); call out the project-lead-only fields with `[project lead: ...]` placeholders.
...
**What does NOT ship in Phase A.** Swift / Kotlin source code (the M3 follow-up §9.47 closed the contract; the implementation is a future Mac / Linux session). iOS / Android device builds (the CI toolchain smoke check validates the SDK paths; a real build requires a code-signing identity + a real device, which is a follow-up slice). App Store / Play Store uploads (the `release-ios` + `release-android` jobs are commented out; the project lead uncomments + configures the signing secrets before the first real mobile release). iOS / Android-specific UI changes (the mobile UI uses the same React shell as desktop; layout reflows are handled by the existing `useViewport` hook + CSS media queries). The actual `secrets_stronghold.rs` facade (with the `Stronghold::create_client` + `client.store().insert(...)` calls) + the `.plugin(tauri_plugin_stronghold::Builder::new(...).build())` wiring in `lib.rs` (deferred to the future Mac / Linux session — these need the actual Android password-flow design, which is project-lead-only).
...
**Key decisions (Phase A).** See the full table in `docs/superpowers/specs/2026-06-16-mobile-build-roadmap-design.md`. Highlights:
...
- **#184** (error mapping reuses existing variants): the `map_stronghold_error` function deliberately reuses the 3-variant `SecretError` enum (the JS-side `SecretErrorPayload` mirrors these exact 3 variants). Adding new variants would be a breaking change to the public IPC contract.
...
- **2 new JSON parse tests** in `src-tauri/src/tauri_config.test.rs` (inside `lib.rs`'s `#[cfg(test)] mod tauri_config`):
...
- **5 new `secrets` tests** in `src-tauri/src/secrets.rs`:
  - `pick_secrets_backend_returns_keyring_for_windows_macos_linux_gtk_ios` — desktop + iOS use `keyring`.
  - `pick_secrets_backend_returns_stronghold_for_android` — Android uses Stronghold (when `mobile` feature is on) or falls back to `keyring` (when it's off, e.g. on the Windows dev build).
  - `pick_secrets_backend_returns_keyring_for_other` — `OsFamily::Other` falls back to `keyring` (mock store).
  - `stronghold_error_maps_to_secrets_error_correctly` (#[cfg(feature = "mobile")]) — 2 cases for the 2 `tauri-plugin-stronghold::Error` variants we pattern-match.
  - `secrets_get_api_key_dispatch_logic_for_android` — smoke test for the dispatch logic.

**Verification (Phase A).** `npx tsc -b` 0 errors · `npx vitest run` 1227/1227 pass (unchanged; no new JS tests) · `npm run build` clean · `cargo check --features m2c-native --lib` clean (desktop build unchanged) · `cargo check --features mobile --lib` clean (Stronghold dep compiles on Windows) · `cargo test --lib` 364/364 pass (was 358 in M6c; +6 new: 2 tauri_config + 4 secrets, plus 1 `#[cfg(feature = "mobile")]`-gated test that only runs on the `mobile` build) · CI `mobile-toolchain` job runs on every PR.

**Future-session work items.** The future Mac / Linux session's checklist is in `docs/plans/mobile-build-roadmap.md`. The 6 steps per platform are: (1) `cargo tauri {ios,android} init`, (2) add the Swift / Kotlin plugin per the contract READMEs, (3) add the `Info.plist` / `AndroidManifest.xml` permission keys, (4) build the `.ipa` / `.aab`, (5) smoke test on a real device, (6) code signing + store submission (project-lead-only). The `secrets_stronghold.rs` facade + the plugin init in `lib.rs` are also future-session tasks (they need the actual Android password-flow design).
...
| 187 | Theme illustrations are loaded via `import.meta.glob('@/shared/assets/themes/*.{png,jpg}', { eager: true, query: '?url', import: 'default' })`, not via individual `import img from '...png'` statements. | `import.meta.glob` returns a `{ [path]: url }` map in one import — Vite hashes + bundles every PNG/JPG in the directory at build time. Adding a future theme is "drop the file in the folder + add a THEMES row"; no source-code wiring beyond the data entry. The `img(filename)` helper in `themes.ts` does a fail-fast lookup that throws a clear error at module load if the file is missing, which catches "I renamed the asset but forgot the data entry" at app boot instead of as a runtime 404 on the active tab. The `eager: true, query: '?url'` combo means the URL strings are inlined at compile time (no async resolver). | 2026-06-18 |
| 188 | The active editor tab's theme illustration is a **CSS-variable swap**, not a real image crop or a canvas-derived image. The picker writes `background-position: var(--theme-img-crop)` (one of 9 hardcoded percentages) on the same image asset the picker card is showing. | Per the Phase 10 budget cap ("10% of dev attention, no real canvas crop, no free-drag picker"), the underlying image is reused as-is and the user picks a "view" not a "crop." The implementation is ~30 lines of CSS — `background-image: var(--theme-img); background-size: cover; background-position: var(--theme-img-crop)` on `.tab[data-active]` — and the only TabStrip code change is the four-property rule (no JS, no IPC, no asset variants). The 9-position grid covers the useful space (top-left, top, top-right, left, center, right, bottom-left, bottom, bottom-right) without the combinatorial explosion of a free-drag picker. A future "premium crop" feature (Phase 11+) can layer a real canvas cropper on top of the same picker UI; the data flow stays the same. | 2026-06-18 |
| 189 | `themeStore` (Phase 10) follows the exact `voicePreferencesStore` pattern: Zustand store + localStorage persistence at key `lipi:theme:v1` + idempotent `hydrate()` + `setupThemePersistence()` called once at app startup in `AppRoot`'s `useEffect`. The store holds `{ themeId, cropIndex, hydrated }`; the side effect (`applyThemeTokens(theme, cropIndex)` writing to `:root` CSS variables) lives in the *subscribe* callback, not in the setters. | Same shape as Decision #48-era voicePreferences, so the future maintenance cost is one store, not two. Side effects in the subscribe callback (vs. in the setter) keep the store a pure state container — the test suite can mutate the store and observe state without triggering DOM writes. The defensive `isValidThemeId` / `isValidCropIndex` guards in `loadFromStorage` reject a malformed localStorage payload (post-quantum-browser-attack, manual edit, schema migration glitch) and fall back to the bundled defaults rather than throwing. The choice of "default theme is hickory-hollow, default crop is center" matches the §"Mood" ordering in `themes.ts` — the warmest, most grounded option is the safe fallback. | 2026-06-18 |
...
### 9.50 Stronghold snapshot password-flow (Android) — DESIGN COMPLETE, IMPLEMENTATION DEFERRED

Phase A (Decision #177) shipped the `secrets_stronghold.rs` facade with a **hardcoded 32-byte placeholder** for the `KeyProvider` key (the bytes of `"Lipi-stronghold-v1-placeholder!!"`). The placeholder is not a secret — anyone who can `adb pull` the snapshot file off the device can decrypt it. The placeholder exists only to satisfy the `KeyProvider::try_from(Zeroizing<Vec<u8>>)` 32-byte length requirement so the seam is buildable + testable on the Windows dev box; it's flagged in the source code as "the v1 password-flow design is the future Mac / Linux session's task."

This entry closes the design loop. **Decision #186** picks the v1 password-flow: **Android Keystore derivation**. A small Kotlin helper (`StrongholdKeyBridge.kt`, sibling to the future `lipi-stt-android` plugin) generates a hardware-backed AES-256 key via the Android Keystore (`KeyGenParameterSpec` + `Cipher` "AES/GCM/NoPadding"), encrypts a 32-byte zero plaintext under that key, and hands the ciphertext back to Rust. The Rust side SHA-256s the ciphertext and uses that as the `KeyProvider` key. The Android Keystore key never leaves the device's TEE / Strongbox (on hardware that supports it), the snapshot file is useless without the Keystore key, and the user sees no UX cost (no password prompt, no biometric per session).

**Why Android Keystore over the other two options.** Per-install-random (Option B) reduces to the same security properties with an extra indirection — both ultimately need the Android Keystore to store anything safely, so the indirection adds complexity without adding security. User-entered-password (Option C) has a high UX cost (user has to remember a password or lose access to all their AI keys) and the low-entropy password problem (most users pick a 6-character lowercase password that's brute-forceable in minutes via GPU Argon2). A hybrid (Option D, Keystore + password) is reserved for a v1.1 follow-up behind a Settings toggle. See `docs/decisions/0186-p-mb-a-stronghold-password-flow.md` for the full analysis (threat model, options, alternatives, implementation plan).

**Trade-off accepted.** Clearing app data wipes both the Android Keystore key and the snapshot file — the user re-enters their AI provider keys. Same on app uninstall without backup, same on device migration. This is acceptable for an IDE (4-5 keys to re-enter is a 30-second task) and is the same trade-off 1Password / Bitwarden / most other Android password managers make. Documented in user-facing copy.

**What's NOT in this entry.** The actual implementation. Kotlin + JNI glue + real-device testing all require an Android physical device + Android Studio Iguana+ on a Linux / Windows box (the future Mac / Linux session's checklist per `docs/plans/mobile-build-roadmap.md`). The implementation steps are listed in § "Implementation plan" of the design doc. The `secrets_stronghold.rs` + Android plugin work is still a future-session task; this decision just removes the design ambiguity so the future session has a clear target.

**Verification (this entry).** No code changed — pure design. The design doc is `docs/decisions/0186-p-mb-a-stronghold-password-flow.md`. The placeholder constant in `secrets_stronghold.rs` is still in place; it's a known v1-only artifact, the comment block on lines 106-117 already documents the follow-up path, and removing the constant is part of the future-session implementation.

---

### 9.51 StrongholdKeyBridge contract — DESIGN + STUB SHIPPED, JNI WIRING DEFERRED

This entry ships the **contract** and the **Rust stub** for D-186's Android Keystore derivation. The actual JNI invocation is still the future Mac / Linux session's task (per §9.50 and the D-186 implementation plan), but the future session now has a concrete starting point: a Kotlin class skeleton with the exact method signature, a Rust module with the JNI call sequence documented as a TODO, and a 3-test unit suite that pins the contract so a refactor on either side breaks loudly.

**Files added.**

- **`docs/plugins/lipi-stronghold-key-bridge/README.md`** — the contract. ~260 lines, 10 sections (goal, targets, gradle module structure, permission flow, the Kotlin class, the Rust JNI binding, testing, migration from v1 placeholder, open questions, references). Mirrors the `lipi-stt-android` plugin README pattern (Phase 0 contract only, no Kotlin code, future session fills in).
- **`docs/plugins/lipi-stronghold-key-bridge/StrongholdKeyBridge.kt`** — the actual Kotlin source skeleton. ~180 lines, with the `package lipi.stronghold` declaration, the `external fun loadKey(): ByteArray` JNI entry point, the `init { System.loadLibrary("lipi_stronghold_key_bridge") }` block, the `loadKeyImpl()` body (Keystore lookup + key gen + AES/GCM/NoPadding encrypt), and the `getOrCreateKey()` helper. The future session copies this file into `android/app/src/main/kotlin/lipi/stronghold/` and starts filling in the gradle wiring.
- **`src-tauri/src/secrets_stronghold_key_bridge.rs`** — the Rust stub. `#[cfg(feature = "mobile")]` gated, with one public function `load_key_from_keystore() -> Result<Zeroizing<Vec<u8>>, SecretError>`. On `target_os = "android"`, returns `SecretError::Platform { detail: "JNI not yet wired (D-186 follow-up; see docs/plugins/lipi-stronghold-key-bridge/README.md §6)." }`. On other targets, returns a "not supported on this target" error. The function body has a 25-line pseudo-code comment showing the exact JNI call sequence the future session needs to wire (`env.FindClass` → `env.GetStaticMethodID` → `env.CallStaticObjectMethod` → cast the `jbyteArray`).
- **`src-tauri/src/lib.rs`** — registers the new module as `pub(crate) mod secrets_stronghold_key_bridge;` (not re-exported, because it's only called internally by `secrets_stronghold::keyprovider()` once the future session swaps that helper).

**What did NOT change.**

- `src-tauri/src/secrets_stronghold.rs` — the `PLACEHOLDER_KEY` constant is still in use, the `keyprovider()` helper still reads from it. The future Mac / Linux session is responsible for the one-function swap (documented in the contract README §8 + the D-186 design doc §"Implementation plan"). Until that swap, the Android build keeps using the v1 placeholder.
- `CHANGELOG.md` — the D-186 design entry (already shipped) is the umbrella; this entry is the contract follow-up.

**Tests added.** 3 new unit tests in `secrets_stronghold_key_bridge::tests`:

- `stub_returns_platform_error_with_documented_message` — pins the contract that the stub returns `SecretError::Platform` with a message containing both `"StrongholdKeyBridge"` and `"D-186"`. A future session that wires the JNI will need to update this test (and the function body).
- `expected_return_length_matches_aes_gcm_iv_plaintext_tag` — pins `EXPECTED_RETURN_LENGTH == 60` (12 IV + 32 plaintext + 16 GCM tag). A future refactor that switches to ChaCha20-Poly1305 (different tag length) breaks the test loudly.
- `class_name_and_signature_match_kotlin_source` — pins `CLASS_NAME == "lipi/stronghold/StrongholdKeyBridge"` and `LOAD_KEY_SIGNATURE == "()[B"`. A Kotlin-side rename (e.g. `StrongholdKeyBridgeV2`) breaks the test loudly rather than failing at runtime.

**Verification (this entry).** `cargo check --lib` (default features) 0 errors, 0 warnings. `cargo check --features mobile --lib` 0 errors, 0 warnings. `cargo test --lib --features mobile secrets_stronghold_key_bridge` 3/3 pass. `cargo test --lib --features mobile` 383/383 pass (was 380 in §9.49; +3 new in this entry). `npx tsc -b` clean, `npx vitest run` 1243/1243 pass (no JS change).

**Future-session checklist (additive on top of the D-186 implementation plan).**

1. Add the `:android-key-bridge` Gradle module (or similar name) to `android/app/build.gradle.kts` per the contract README §3.
2. Copy `docs/plugins/lipi-stronghold-key-bridge/StrongholdKeyBridge.kt` to `android/app/src/main/kotlin/lipi/stronghold/StrongholdKeyBridge.kt`.
3. Add a `cdylib` target for the Rust JNI implementation (either as a new crate in a workspace or as a target in the main `src-tauri/Cargo.toml`). See the contract README §9 for the trade-off.
4. Implement the JNI call sequence in `secrets_stronghold_key_bridge::load_key_from_keystore_android` per the pseudo-code in the file's TODO.
5. Wire the `lipi_stronghold_key_bridge` function into `secrets_stronghold::keyprovider()` per the contract README §8.
6. Add the `StrongholdKeyBridgeTest.kt` Robolectric test + a real-device end-to-end test (keystore key survives an app restart).
7. Remove the `PLACEHOLDER_KEY` constant from `secrets_stronghold.rs` (or keep as a documented fallback for broken-Keystore devices — v1.1 follow-up).

---

### 9.52 Phase 10 — SHIPPED (Editor tab theme: 5 vintage scenes, see CHANGELOG "Added (Phase 10 — Editor tab theme: 5 vintage scenes)")

Phase 10 ships the **Appearance → Theme** section in Settings. The user picks one of 5 vintage nature-print illustrations (Hickory Hollow, Whispering Pines, Marigold Field, Wildflower Field, Quiet Valley) and a 9-position frame crop; the active editor tab is painted with the cropped image while the rail, file tree, editor, AI panel, and status bar stay in the dark-first IDE neutral. The selection persists across reloads via `lipi:theme:v1` localStorage.

**Architecture in one paragraph.** `themes.ts` owns the `Theme` interface + `THEMES` array + `CROP_POSITIONS` constants + `applyThemeTokens()` (the single function that writes `--theme-img` / `--theme-img-crop` / `--theme-accent` / `--theme-accent-soft` to `:root`). Illustrations load via `import.meta.glob` — fail-fast if a file is missing. `themeStore.ts` is a Zustand store + persistence, mirroring the `voicePreferencesStore` pattern (Decision #189). `ThemeSection.tsx` (Settings → Theme block) is a pure view over the store; `ThemeCard.tsx` + `CropPicker.tsx` are the two picker primitives in `src/shared/components/`. The active editor tab in `EditorWorkspace/TabStrip` reads the four CSS variables — zero JS coupling between the picker and the tab, no IPC, no asset variants.

**Files added (10 new).**

- **`src/shared/state/themes.ts`** — `Theme` interface, `THEMES` (5 entries with id / name / mood / imageUrl / accent / accentSoft), `CROP_POSITIONS` (9), `CROP_LABELS` (9), `DEFAULT_THEME_ID`, `DEFAULT_CROP_INDEX`, `findTheme`, `cropAt`, `applyThemeTokens` (the CSS-variable bridge), and `THEME_CSS_VARS` (the four exported CSS-variable names). `import.meta.glob` loads every PNG/JPG in `src/shared/assets/themes/` at build time and the `img(filename)` helper does a fail-fast lookup.
- **`src/shared/state/themeStore.ts`** — Zustand store mirroring `voicePreferencesStore`. Holds `{ themeId, cropIndex, hydrated }`. Persists to `lipi:theme:v1`. Defensive `isValidThemeId` / `isValidCropIndex` validators reject malformed payloads and fall back to defaults. Exports `useThemeStore`, `themeSelectors`, `setupThemePersistence` (the subscribe-once wiring that also calls `applyThemeTokens` on every change).
- **`src/shared/state/themeStore.test.ts`** — 21 vitest tests. Defaults (hickory-hollow + center). `setThemeId` updates + resets crop + rejects invalid ids. `setCropIndex` accepts all 9 positions + rejects out-of-range / non-numbers. `resetCrop` returns to center. `hydrate` reads valid persisted state + rejects malformed JSON / unknown theme id / out-of-range crop / wrong shape + is idempotent + flips the `hydrated` flag. Persistence round-trips after `setupThemePersistence`. Selectors return current values. All 5 theme ids + all 9 crop positions accepted.
- **`src/shared/assets/themes/01-hickory-hollow.jpg`** — autumn leaves + acorn vintage botanical print. 155 KB.
- **`src/shared/assets/themes/02-whispering-pines.jpg`** — moonlit trees, vintage poster. 189 KB.
- **`src/shared/assets/themes/03-marigold-field.jpg`** — whimsical girl in golden wheat field with butterfly. 250 KB.
- **`src/shared/assets/themes/04-wildflower-field.jpg`** — white daisies on blue sky, vintage botanical print. 302 KB.
- **`src/shared/assets/themes/05-quiet-valley.jpg`** — green mountains + river, vintage Ghibli-style. 232 KB.
- **`src/shared/components/ThemeCard/ThemeCard.tsx`** — single theme card. `<button type="button">` with `aria-pressed`, per-card `--card-accent` / `--card-accent-soft` inline-styled. `data-active` attribute drives the active border + checkmark badge.
- **`src/shared/components/ThemeCard/ThemeCard.module.css`** — 4/5 aspect-ratio art container, 2-line-clamped mood line, accent border on `:focus-visible` and `[data-active]`, 28×28 checkmark badge with `var(--card-accent)` background.
- **`src/shared/components/ThemeCard/index.ts`** — barrel.
- **`src/shared/components/CropPicker/CropPicker.tsx`** — 3×3 grid of 32×32 thumbs (`role="radiogroup"` / `role="radio"` / `aria-checked`), each thumb showing the active theme's image cropped to its position. Reset button + `aria-live="polite"` crop-name readout.
- **`src/shared/components/CropPicker/CropPicker.module.css`** — 32 px thumbs in a fixed 96×96 grid (`grid-template-columns: repeat(3, auto)` × 3 rows), `box-shadow` ring on active + `:focus-visible`, Reset button with `:disabled` state when crop is already center.
- **`src/shared/components/CropPicker/index.ts`** — barrel.
- **`src/screens/SettingsProvider/components/ThemeSection/ThemeSection.tsx`** — Appearance → Theme block. Reads from `useThemeStore` via the narrow `themeSelectors` (Decision #6 / Rule 6 — no over-broad subscriptions). Composes 5 `<ThemeCard>` + `<CropPicker>` + a "Reset to center" link. The store's `setThemeId` resets the crop on its own — the section just renders.
- **`src/screens/SettingsProvider/components/ThemeSection/ThemeSection.module.css`** — section heading + lede + responsive theme grid (auto-fit `minmax(220px, 1fr)` on desktop, 2 cols below 900px, 1 col below 480px — matches the surrounding settings cards' rhythm).

**Files modified (3).**

- **`src/shared/components/index.ts`** — added `export * from './ThemeCard';` and `export * from './CropPicker';` so consumers import from the barrel, not from individual component paths (per Rule 4).
- **`src/screens/EditorWorkspace/components/TabStrip/TabStrip.module.css`** — `.tab[data-active]` now reads `background-image: var(--theme-img)` + `background-position: var(--theme-img-crop)`. Added the dark wash overlay (`::before`, `linear-gradient(180deg, rgba(11,13,16,0.35), rgba(11,13,16,0.85))`) for legibility, the 2 px accent stripe on top (`::after`, `var(--theme-accent)`), and the children-need-z-index-1 rule so the file name stays crisp over the illustration. `.dot[data-dirty]` now uses `var(--theme-accent)` so a Hickory Hollow session has warm-brown dots and a Whispering Pines session has forest-green dots. **No `.tsx` changes** — TabStrip.tsx was untouched.
- **`src/screens/SettingsProvider/SettingsProvider.tsx`** — imported `ThemeSection` and inserted `<ThemeSection />` between `<LanguageServerCard />` and the AI Tools `<h2>` (the natural slot — Theme is an editor-appearance feature, not AI or Voice).

**Tests added (21).** All in `src/shared/state/themeStore.test.ts`. See the file's header comment for the test-by-test breakdown; coverage spans defaults, all setters, all defensive guards, all hydrate paths, persistence round-trips, all 5 theme ids × all 9 crop positions.

**Verification (this entry).** `npx tsc -b` exit 0. `npx vitest run` 98 files, 1264 tests pass (was 97/1243 in §9.51; +1 file = themeStore.test.ts with 21 new tests). `cargo check` (default features) exit 0. The monaco-editor marked.js source-map warning in the vitest output is pre-existing — same noise from §9.51, unrelated to Phase 10.

**What did NOT change.** Monaco editor's editor.tsx (no theme integration into the editor body — only the tab strip gets the illustration). The file tree's active-row background still uses `--color-accent-soft` (the muted project accent), not `--theme-accent-soft` — matching Phase 10's "illustration only paints the active tab" UX contract from the picker card copy. The Rust side (no theme assets on the Rust side; theme images go straight to the JS bundle). The picker card's `accent-soft` color is exposed via the per-card inline style but isn't used in the picker itself — it's there for the future "tree active-row uses theme accent" polish in §10.3.

**Follow-up slices (out of scope for Phase 10).** Tracked in `docs/plans/theme-feature.md` (TODO — write after Phase 10 ships).

1. **Free-drag cropper** (Phase 11+). The current 9-position grid is the v1 surface. A real drag-and-zoom picker (React-PDF-style or `react-image-crop`) is a 2-3 day slice; the data flow (crop stored as `x, y, scale, rotation` vs. integer index) is the only architectural change. The current `CROP_POSITIONS` constant stays as the "presets" entry in the new picker.
2. **Custom theme upload** (Phase 11+). User picks an image from disk → resize to 4:5 aspect → write to `src/shared/assets/themes/` (or a per-workspace `lipi-themes/` dir) → auto-add to `THEMES`. Currently `THEMES` is a `readonly` array of fixed ids; the data source would need to become a registry (same shape as the LSP server registry in Phase 9).
3. **Quick-cycle button in the top bar** (Phase 11+). A `ThemeButton` next to the search box in the title bar that cycles through `THEMES` on click. ~30 lines; reads `useThemeStore` + writes `setThemeId`. Mirrors the M3-era `VoiceButton` pattern.
4. **Tree active-row uses `--theme-accent-soft`** (already in the data — just wire the CSS rule). One-line change in `FileTreePane.module.css`.
5. **"Active file" pane preview** — a tiny preview of the cropped theme image in the file tree's tooltip when the user hovers the active tab. ~50 lines, low value, defer unless users ask.
