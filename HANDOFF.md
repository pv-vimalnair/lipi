...
The M6b design (`docs/plans/m6b-design.md`) explicitly parked scroll and cursor memory as out-of-scope — Decision #81's "all per-tab state is in `WorkspaceTabState`" invariant was preserved, but the per-file cursor and per-tab file-tree scroll position were left for a follow-up. M6c is that follow-up: the minimal slice that picks up the two scroll/cursor items, leaving the M6b non-goals list (per-tab font / theme / recents / tool / voice / git / search settings) authoritative for a later phase.

The data model is `WorkspaceTabState` extended with `editorCursorByPath: Record<filePath, { line, column }>` and `fileTreeScrollAnchor: string | null` (the topmost visible row's `data-tree-path`). The mirror-back architecture is two `useEffect`s per owning component: `EditorPane.tsx`'s `ActiveEditor` subscribes to `editor.onDidChangeCursorPosition` (debounced via `requestIdleCallback` / `setTimeout(500ms)` fallback in `scheduleCursorMirrorBack`) and writes to `useWorkspaceStore.setEditorCursor`; the same component rehydrates the saved cursor via `editor.setPosition` + `revealPositionInCenterIfOutsideViewport` on `onDidChangeModel`, guarded by a `suppressNextCursorChange` ref to prevent the rehydrate itself from triggering a mirror-back round-trip. `FileTreePane.tsx`'s `TreeRoot` does the symmetric thing: rows carry `data-tree-path={entry.path}`, the rehydrate `useEffect` waits one rAF (with up to 2 retries for async `entriesByDir` loads) and calls `row.scrollIntoView({ block: 'start' })`, the mirror-back `useEffect` attaches a passive `scroll` listener throttled to rAF, finds the first row whose bottom is below the container top + 1px, and writes the path with a transition-only write guard. Both effects unmount-flush synchronously so the user's last position is always persisted.

The settings export/import format bumps from v4 to v5 (`src/shared/settingsIOv5.ts` + the matching `apply` and `preview` modules). v4 → v5 in-memory migration synthesises the 2 new fields with empty defaults; v3 → v4 → v5 chains reuse the v4 module's helpers. The `PrivacyDataCard` now exports in v5 format and shows the user a notice that v3 / v4 imports are upgraded on load (with empty per-file cursor / scroll). Decisions #167–#171 cover the architectural choices (the mirror-back pattern, the throttle, the v5 schema, the PrivacyDataCard UX, and the `data-tree-path` DOM marker). The M6b non-goals list is still authoritative; M6c is the minimal slice that picks up the two scroll/cursor items.
...
| 168 | Cursor + scroll mirror-back is in the component that owns the DOM ref (`ActiveEditor` / `TreeRoot`), not in the store-level hooks (`useEditorTabs` / `useFileTree`). | Imperative DOM access (Monaco's `editor.setPosition`, `row.scrollIntoView`, passive `scroll` listener) needs a component-level ref. The store-level hooks stay focused on store-level mutations; mixing the two would couple `useFileTree` to DOM events. | The "where does the scroll listener live" choice is a single point of design; a future "scroll the tree on search-result click" feature would extend the existing `TreeRoot` effect, not a new store-level one. |
| 169 | `scheduleCursorMirrorBack` uses `requestIdleCallback` with a `setTimeout(500ms)` fallback, and exposes a synchronous `_flushPendingCursor` for unmount-time flushing. | `requestIdleCallback` is the right primitive (write happens on idle, not on a fixed schedule); the `setTimeout` fallback is for jsdom (no `requestIdleCallback`); the synchronous flush is for the unmount cleanup to guarantee the user's last position is persisted. | None — the throttle is well-isolated, the fallback is tested, and the flush is exercised by the `unmount flushes the pending cursor` test. |
| 170 | The v5 settings format is `LIPI_STATE_V5_FORMAT = 'lipi-state'`, `LIPI_STATE_V5_VERSION = 5`. | Matches the M6b format-string convention (`'lipi-state'`) and bumps the version to 5. The format-string stays stable across versions; only the version field changes. | The v5 → v6 (or higher) migration, when it lands, will need a similar chain. |
| 171 | `data-tree-path={entry.path}` is the file-tree row's DOM marker for scroll anchor lookups. | One attribute, one selector; no need for a ref-map (which would couple `TreeRoot` to React's reconciliation order). The selector uses `CSS.escape(anchor)` to safely embed paths that may contain `.` or `\\`. | jsdom doesn't implement `CSS.escape`; the polyfill lives in `vitest.setup.ts` (not in production code). Production is unaffected. |
