...
*End of handoff. Lipi is at **D-145 complete** (single Monaco instance across tab switches — `EditorPane` dropped `key={activeTab.id}` so one Monaco instance persists for the pane's lifetime; the `@monaco-editor/react` `Editor` component handles the model swap in place via `editor.setModel(model)` on `path` change; the `[content]` effect is now path-aware (D-149) and skips `setValue` on tab switch; the `pendingReveal` check moved from `handleMount` to an `onDidChangeModel` subscription (D-150) so it fires on every model swap, not just the first mount; the `setControllerEditor(null)` cleanup is now real-unmount-only (D-151), not on every tab switch; 5 new tests in `EditorPane.test.tsx` pin the contract — controller-store stability, no editor remount on tab switch, bridge aggregator doesn't re-run, cleanup still works on real unmount; total 1167 TS tests, vitest + tsc + cargo check all clean). The next session should resume from the **D-146 follow-up** (provider respawn re-registration), then the parked M6c / M3 follow-up / mobile-build roadmap, plus the two items from §9.30 (MSI bundling regression, `whisper-rs` bump) and the JSON/CSS/HTML workers follow-up in §9.31. The next handoff entry will live at §9.47.*
