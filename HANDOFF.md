...
*End of handoff. Lipi is at **D-146 complete** (LSP provider re-registration on respawn — `useMonacoLspBridge` subscribes to `lspClientStore`'s `clients` map, reacts to `handleId` changes per kind, disposes the old provider set and re-registers a fresh one against the new client; `registerProvidersForKind` is an extracted async function with `cancelled` checks; optimistic `lastHandleIdsByKind` update breaks the `setState` cascade loop; first-observation guard prevents double-registration on mount; the test mock's `lspStdioRead` now uses a per-`handleId` queue to mirror the Rust side's per-handle stdout delivery — D-146's respawn test exposed the single-FIFO race; 3 new tests in `useMonacoLspBridge.test.tsx` pin the contract; total 1170 TS tests, vitest + tsc + cargo check all clean). The next session should resume from the **MSI bundling regression** (§9.30 follow-up), then the **`whisper-rs` bump**, then the **JSON / CSS / HTML Monaco workers** (§9.31), then **M6c per-tab UI state** (scroll / font / theme / recents / git / tool / voice), then the **M3 Swift `SFSpeechRecognizer` plugin** follow-up, then the **mobile-build roadmap**.*
