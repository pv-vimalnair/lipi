...
**Current phase: Phase 4 (of the production-readiness roadmap) — IAP receipt validation — SHIPPED.** This is the **last code-focused phase** before public distribution. Phase 4 fills in the `iap_redeem` stub from Phase 3 with real Apple App Store `verifyReceipt` validation (`https://buy.itunes.apple.com/verifyReceipt`) and Microsoft Store Broker API receipt validation. The Phase 3 `iap_redeem` Tauri command is rewritten as a **dispatcher** that inspects the receipt format (JSON → Apple, XML → Microsoft) and routes to the matching platform validator. On success, the dispatcher generates a `LicensePayload` bound to the current machine, signs it with a **per-machine Ed25519 keypair** (generated on first IAP redemption, stored in the OS keychain under `app.lipi.ide / iap-privkey` + `app.lipi.ide / iap-pubkey` — the privkey never leaves the machine), and saves the license to the keychain (the same code path as `license_activate`). The `LicensePayload` is extended with a `kid` (key id) field that identifies which pubkey to use to verify the signature: `"trial"` → embedded trial pubkey, `"offline"` → embedded production pubkey (for `LIP1...` keys from purchase emails), `"iap-local"` → per-machine pubkey from the keychain. The verifier dispatches on `kid`. The shared secret for Apple's `verifyReceipt` is read at build time from the `LIPI_APPLE_IAP_SHARED_SECRET` env var via `option_env!` (so the binary never has the secret on disk in plaintext); if unset, every call returns `iap-shared-secret-missing`. The Microsoft OAuth flow is stubbed in Phase 4 (the production bearer token is read from `LIPI_MS_IAP_BEARER_TOKEN`); a full OAuth client-credentials flow is a v1.1 follow-up. The new `LicenseError::MissingLocalPubkey` variant is returned when an IAP-issued license references a per-machine pubkey that's no longer in the keychain (OS reinstall, keychain wipe) — the UI humanizes this reason with a "re-run the IAP flow" message. `humanizeInvalidReason` is updated to handle all the new `iap-*` reason codes (`iap-receipt-format-unrecognized`, `iap-sandbox-not-supported`, `iap-product-id-mismatch`, `iap-expired`, `iap-network-error`, `iap-keychain-error`, etc.). The IPC surface (`iap_redeem(receipt, plan)`) is unchanged from Phase 3; only the Rust implementation changed. The UI doesn't need to change. `cargo test --lib` is **292/292 passing** (+53 from this phase: 12 iap_apple + 13 iap_microsoft + 9 iap_keypair + 13 iap_dispatcher + 6 pre-existing round-trip tests re-exercised with the new `kid` field); `npx vitest run` is **985/985 passing** (+12 from this phase: 5 iap.test.ts + 7 LicenseCard.test.ts IAP paths); `tsc --noEmit` / `npm run build` / `cargo check` all clean. See `CHANGELOG.md` "Added (Phase 4 — IAP receipt validation)" for the full feature list; see HANDOFF §9.27 for the per-phase writeup; see Decisions #97–#99 for the architectural calls. **The production-readiness roadmap is now COMPLETE.** The only remaining work is the project lead's non-code setup (LLC formation, ToS, marketing site, support rotation) — those are the project lead's own work, not code, and run in parallel from the project lead's side.
