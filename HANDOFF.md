...
*End of handoff. Lipi is at **Phase 6.2 complete** (`whisper-rs` 0.14 → 0.16 — the m2c-native build now succeeds end-to-end: `cargo check --features m2c-native --lib` is clean, `cargo test --lib --features m2c-native` is 358/358 pass, and the default-feature build is still 358/358 pass; `npx tsc --noEmit` is 0 errors, vitest is 1185/1185 pass). The next session should resume from **M6c per-tab UI state** (scroll / font / theme / recents / git / tool / voice), then the **M3 Swift `SFSpeechRecognizer` plugin** follow-up, then the **mobile-build roadmap**.*
